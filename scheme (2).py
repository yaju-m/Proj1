...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        #eval operator, eval operancs, apply procedure on eval operands
        #raises error if provided arg not scheme procedure
        #map method of Pair applies func to every item in scheme list
        #scheme apply applies scheme to some arg
        def helper(n_expr):
            return scheme_eval(n_expr, env)

        operator= scheme_eval(first, env)
        check_procedure(operator)
        operands= expr.second.map(helper)
...
def eval_all(expressions, env):
    ...
    # BEGIN PROBLEM 8
    #if passed an empty list of expressions (nil) return None
    if expressions == nil: 
        return None  
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals): 
            raise SchemeError('Number of values doesnt match')
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        #correctly handles shorthand
        function_name= target.first
        env.define(function_name, do_lambda_form(Pair(target.second, expressions.second), env))
...
def do_quote_form(expressions, env):
    ...
    # BEGIN PROBLEM 7
    #exp is given as Pair(A, nil) where A is the quotes expression
...
def do_begin_form(expressions, env):
    ...
    #call eval_all to complet begin
    #begin evals all sub exp in order, value is value of final subexp
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 9
    #creates LambdaProcedure instances
    #body of a lambda procedure: scheme list of body expressions
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    def check_each(bindings):
        if bindings== nil: 
            return nil
        check_form(bindings.first, 2, 2)
        check_each(bindings.second)
    check_each(bindings)
    ...
    def make_formals(bindings):
        if bindings == nil: 
            return nil
        return Pair(bindings.first.first, make_formals(bindings.second))
    list_of_formals = make_formals(bindings)
    check_formals(list_of_formals)
    def make_vals(bindings):
        if bindings == nil: 
            return nil
        return Pair(scheme_eval(bindings.first.second.first, env), make_vals(bindings.second))
    list_of_vals = make_vals(bindings)
    return env.make_child_frame(list_of_formals, list_of_vals)
    # END PROBLEM 15
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            # BEGIN Extra Credit
            def helper(n_expr):
                return scheme_optimized_eval(n_expr, env)
